...
class HoldemTable(Env):
    ...
    def _check_game_over(self):
        """Check if only one player has money left or this hand ends"""
        player_alive = []
    ...
    def _game_over(self):
        ...
        # best_player = 0
        # biggest_stack = self.players[0].stack
        # for i in range(len(self.players)):
        #     if self.players[i].stack >= biggest_stack:
        #         biggest_stack = self.players[i].stack
        #         best_player = i
        ...
        # print("Winner : {}".format(self.players[self.winner_ix].name))
        # log.info(f"Winner: {self.winner_ix}")
        # print("GAME OVER")
        # time.sleep(5)
