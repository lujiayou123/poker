...
class Player:
    ...
    def action(self, action_space, observation, info):  # pylint: disable=no-self-use
        ...
        elif equity_alive > self.min_bet_equity + incremen1 and Action.RAISE_200_POT in action_space:
            action = Action.RAISE_200_POT

        elif equity_alive > self.min_bet_equity and Action.RAISE_100_POT in action_space:
            action = Action.RAISE_100_POT

        elif equity_alive > self.min_bet_equity - incremen1 and Action.RAISE_50_POT in action_space:
            action = Action.RAISE_50_POT
