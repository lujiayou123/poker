...
class MonteCarlo(object):
    ...
    def get_two_short_notation(self, input_cards, add_O_to_pairs=False):#把AD KD变成AKS
    ...
    def distribute_cards_to_players(self, deck, player_amount, player_card_list, known_table_cards,
        ...
        #player_card_list,所有玩家手牌的list,[['2D', '7D'],[[XX,XX]]
        ...
        for player_cards in player_card_list:
            ...
            if type(player_cards) == set:
                while True:
                    passes += 1
                    random_card1 = np.random.randint(0, len(deck))#牌的index
    ...
    def distribute_cards_to_table(self, Deck, table_card_list):
        ...
        # print("5cards:{}".format(table_card_list))
    ...
    def run_montecarlo(self, original_player_card_list, original_table_card_list, player_amount, ui, maxRuns,
        ...
        if type(opponent_range) == float or type(opponent_range) == int:
            ...
            # print(type(opponent_allowed_cards))
            # print(opponent_allowed_cards)
...
def get_equity(player_cards, table_cards, players, runs, range):
    """Get equity from a monteacrlo run"""
    simulation = MonteCarlo()
    simulation.run_montecarlo([list(player_cards)], list(table_cards), players, 1, maxRuns=runs,
                              timeout=time.time() + 1, ghost_cards='', opponent_range=range)
    return simulation.equity

def get_short_notation(input_cards):
    simulation = MonteCarlo()
    crd1 , crd2 = simulation.get_two_short_notation(input_cards)
    return crd1 , crd2

def get_open_range(opponent_range):
    simulation = MonteCarlo()
    range = simulation.get_opponent_allowed_cards_list(opponent_range)
    return range

if __name__ == '__main__':
    print(get_short_notation(['2D', 'AD']))
    print(get_open_range(0.1))
    time.sleep(111)
    Simulation = MonteCarlo()
    # print(Simulation.get_opponent_allowed_cards_list(0.5))
    # time.sleep(111)
    ...
    ghost_cards = ''
    # opp_cards = [['TD', 'JD']]
    # range1, range2= Simulation.get_two_short_notation(opp_cards[0])
    # range=set()
    # range.add(range1)
    Simulation.run_montecarlo(my_cards, cards_on_table, player_amount=players, ui=None, maxRuns=maxruns,
                              ghost_cards=ghost_cards, timeout=timeout, opponent_range=0.1)
